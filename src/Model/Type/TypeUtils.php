...
/**
 * Utilities for inspecting and transforming {@see Type} trees.
 *
...
{
    public static function unwrapDecorated(?Type $expr): ?Type
    {
    ...
    public static function allowsNull(Type $expr): bool
    {
    ...
    public static function preserveNullableUnion(?Type $original, Type $replacement): Type
    {
        if ($original === null) {
            return $replacement;
        }
     ...
     *
     * @param callable(Type): ?Type $rewrite
     */
    public static function rewrite(?Type $expr, callable $rewrite): ?Type
    {
        if ($expr === null) {
            return null;
        }
        ...
        if ($expr instanceof DecoratedType) {
            ...
            $expr->types = array_map(
                static fn(Type $type): Type => self::rewrite($type, $rewrite) ?? $type,
                $expr->types
            );
     ...
     */
    public static function decorateNonNullBranches(?Type $expr, callable $mutate): ?Type
    {
        if ($expr === null) {
            return null;
        }
        ...
        if ($expr instanceof UnionType) {
            $expr->types = array_map(
                static fn(Type $t): Type => self::decorateNonNullBranches($t, $mutate) ?? $t,
                $expr->types
            );
     ...
     */
    public static function normalizeUnion(array $types): Type
    {
        $flat = [];
     ...
     */
    private static function fingerprint(Type $expr): string
    {
        if ($expr instanceof BuiltinType) {
            return 'b:' . $expr->name;
        }
        if ($expr instanceof UnknownType) {
            return 'unknown';
        }
        ...
        if ($expr instanceof InlineObjectType) {
            return 'o:' . $expr->shape->getId();
        }
        if ($expr instanceof IntersectionType) {
            $inner = array_map(static fn(Type $t) => self::fingerprint($t), $expr->types);
            sort($inner);
    ...
    public static function mergeTypeConstraintsAndAnnotations(?Type $current, ?Type $next): ?Type
    {
        if ($next === null) {
            return $current;
        }
        ...
        if ($current instanceof DecoratedType && $next instanceof DecoratedType) {
            self::mergeTypeConstraints($next->constraints, $current->constraints);
            $next->annotations = self::mergeTypeAnnotations($next->annotations, $current->annotations);

            return $next;
        }

        if ($current instanceof DecoratedType) {
